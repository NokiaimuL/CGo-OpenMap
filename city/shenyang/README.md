...
沈阳城市脚本通过 `getStationLabelStyle(station, stationId)` 为换乘站选择站名样式。当前规则是：所有 `type: "tsf"` 的换乘站启用 `"callout"`，但“合作街”保留默认标签样式。

`"callout"` 标签包含文本框底部描边，并由 SVG 连线连接站点中心与文本框左下角、右下角中距离较近的一点。连线位于 `lines-layer`，会随地图缩放和平移同步变换；样式规则位于本目录的 `style.css`。
